...
export const BasketProvider = ({ children }) => {
  ...
  const removeItem = (item) => {
    ...
    setBasketItems(basketItems.filter((currentItem) => currentItem !== item))
  }
  const clearBasket = () => {
    setBasketItems([]);
  }
  ...
  const CalculateSubtotal = () => Math.round(CalculateTotal() / 1.09 * 100) / 100;
  ...
  return (
    <BasketContext.Provider value={[basketItems, addItem, removeItem, prices, clearBasket]}>
      {children}
